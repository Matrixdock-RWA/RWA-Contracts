...
const DELAY_MAX = 59; // MTokenMinter.DELAY_MAX

describe("MTokenMinter", function () {
    ...
    async function deployTestFixture() {
        ...
        await xagm.transfer(alice.address, _rwa(1_000_000n));

        const MTokenMinter = await ethers.getContractFactory("MTokenMinter");
        const minter = await upgrades.deployProxy(MTokenMinter,
            [
                owner.address,
                poolA.address,
                poolB.address,
                [usdt.target],  // tokensAcceptedByA
                [xagm.target],  // tokensAcceptedByB
            ],
            { kind: "uups" },
        );

        return {
            minter, usdt, usdc, xagm, xaum,
    ...
    describe("initialize & configuration", function () {
        ...
        it("sets owner, pools and accepted tokens", async function () {
            const { minter, usdt, usdc, xagm, xaum, owner, poolA, poolB } =
            ...
            expect(await minter.acceptedByB(xagm.target)).to.equal(true);
            expect(await minter.acceptedByB(xaum.target)).to.equal(false);
        });

        it("initializes with multiple / empty accepted-token lists", async function () {
            const { usdt, usdc, xagm, xaum, owner, poolA, poolB } =
                await loadFixture(deployTestFixture);

            const MTokenMinter = await ethers.getContractFactory("MTokenMinter");

            // several tokens per pool (multi-iteration init loops)
            const multi = await upgrades.deployProxy(MTokenMinter,
                [owner.address, poolA.address, poolB.address,
                 [usdt.target, usdc.target], [xagm.target, xaum.target]],
                { kind: "uups" },
            );
            ...
            const empty = await upgrades.deployProxy(MTokenMinter,
                [owner.address, poolA.address, poolB.address, [], []],
                { kind: "uups" },
            );
            expect(await empty.acceptedByA(usdt.target)).to.equal(false);
            expect(await empty.acceptedByB(xagm.target)).to.equal(false);
        });
        ...
        for (const { setter, event, getter, tokenKey } of [
            ...
            { setter: "setAcceptedByB", event: "SetAcceptedByB", getter: "acceptedByB", tokenKey: "xaum" },
        ]) {
            describe(setter, function () {
    ...
    describe("requestToMint", function () {
        ...
        it("transfers the stablecoin to poolAccountA and emits MintRequest", async function () {
            const { minter, usdt, xagm, poolA, alice } = await loadFixture(deployTestFixture);
            ...
            await expect(
                minter.connect(alice).requestToMint(
                    usdt.target, xagm.target, amount, 123n, 45n, now, "0x1234")
            ).to.emit(minter, "MintRequest")
                .withArgs(usdt.target, xagm.target, alice.address, poolA.address,
        ...
        it("uses the current poolAccountA even after it is changed", async function () {
            const { minter, usdt, xagm, alice, other } = await loadFixture(deployTestFixture);
            ...
            await expect(
                minter.connect(alice).requestToMint(
                    usdt.target, xagm.target, amount, 0, 0, now, "0x")
            ).to.emit(minter, "MintRequest")
                .withArgs(usdt.target, xagm.target, alice.address, other.address,
                          amount, 0, 0, "0x");
        ...
        it("reverts when the transferred token is not accepted by pool A", async function () {
            const { minter, usdc, xagm, alice } = await loadFixture(deployTestFixture);
            const now = await time.latest();
            await expect(
                minter.connect(alice).requestToMint(
                    usdc.target, xagm.target, amount, 0, 0, now, "0x")
            ).to.be.revertedWith("INVALID_TOKEN_FOR_MINTING");
        });

        it("honors setAcceptedByA toggling", async function () {
            const { minter, usdc, xagm, poolA, alice } = await loadFixture(deployTestFixture);
            ...
            await expect(
                minter.connect(alice).requestToMint(usdc.target, xagm.target, amount, 0, 0, now, "0x")
            ).to.be.revertedWith("INVALID_TOKEN_FOR_MINTING");
        ...
        it("accepts a timestamp at the DELAY_MAX boundary and rejects one past it", async function () {
            const { minter, usdt, xagm, alice } = await loadFixture(deployTestFixture);
            ...
            await expect(
                minter.connect(alice).requestToMint(
                    usdt.target, xagm.target, amount, 0, 0, T1 - DELAY_MAX, "0x")
            ).to.emit(minter, "MintRequest");
            ...
            await expect(
                minter.connect(alice).requestToMint(
                    usdt.target, xagm.target, amount, 0, 0, T2 - DELAY_MAX - 1, "0x")
        ...
        it("accepts a future timestamp", async function () {
            ...
            await expect(
                minter.connect(alice).requestToMint(
                    usdt.target, xagm.target, amount, 0, 0, future, "0x")
            ).to.emit(minter, "MintRequest");
        });
        ...
        it("reverts when allowance is insufficient", async function () {
            ...
            // no approve() call
            await expect(
                minter.connect(alice).requestToMint(usdt.target, xagm.target, amount, 0, 0, now, "0x")
            ).to.be.revertedWithCustomError(usdt, "ERC20InsufficientAllowance");
        });

        it("reverts when balance is insufficient", async function () {
            const { minter, usdt, xagm, bob } = await loadFixture(deployTestFixture);
            // bob has no usdt but approves anyway
            await usdt.connect(bob).approve(minter.target, amount);
            const now = await time.latest();
            await expect(
                minter.connect(bob).requestToMint(usdt.target, xagm.target, amount, 0, 0, now, "0x")
    ...
    describe("requestToRedeem", function () {
        ...
        it("transfers the rwa token to poolAccountB and emits RedeemRequest", async function () {
            ...
            await expect(
                minter.connect(alice).requestToRedeem(
                    xagm.target, usdt.target, amount, 7n, 8n, now, "0xbeef")
            ).to.emit(minter, "RedeemRequest")
                .withArgs(xagm.target, usdt.target, alice.address, poolB.address,
                          amount, 7n, 8n, "0xbeef");

            expect(await xagm.balanceOf(poolB.address)).to.equal(amount);
            expect(await xagm.balanceOf(alice.address)).to.equal(_rwa(1_000_000n) - amount);
        });

        it("uses the current poolAccountB even after it is changed", async function () {
            const { minter, xagm, usdt, alice, other } = await loadFixture(deployTestFixture);
            ...
            await expect(
                ...
                .withArgs(xagm.target, usdt.target, alice.address, other.address,
                          amount, 0, 0, "0x");
            expect(await xagm.balanceOf(other.address)).to.equal(amount);
        });
        ...
        it("honors setAcceptedByB toggling", async function () {
            ...
            await xaum.transfer(alice.address, amount);
            ...
            await minter.setAcceptedByB(xaum.target, true);
            now = await time.latest();
            await expect(
                minter.connect(alice).requestToRedeem(xaum.target, usdt.target, amount, 0, 0, now, "0x")
            ).to.emit(minter, "RedeemRequest");
        ...
        it("accepts a timestamp at the DELAY_MAX boundary and rejects one past it", async function () {
            ...
            await xagm.connect(alice).approve(minter.target, amount * 2n);

            const T1 = (await time.latest()) + 1000;
            await time.setNextBlockTimestamp(T1);
            await expect(
                minter.connect(alice).requestToRedeem(
                    xagm.target, usdt.target, amount, 0, 0, T1 - DELAY_MAX, "0x")
            ).to.emit(minter, "RedeemRequest");
            ...
            await expect(
                minter.connect(alice).requestToRedeem(
                    xagm.target, usdt.target, amount, 0, 0, T2 - DELAY_MAX - 1, "0x")
            ).to.be.revertedWith("INVALID_TIMESTAMP");
        });
        ...
        it("reverts when allowance is insufficient", async function () {
            ...
            ).to.be.revertedWithCustomError(xagm, "ERC20InsufficientAllowance");
        });

        it("reverts when balance is insufficient", async function () {
            const { minter, xagm, usdt, bob } = await loadFixture(deployTestFixture);
            await xagm.connect(bob).approve(minter.target, amount);
            const now = await time.latest();
            await expect(
                minter.connect(bob).requestToRedeem(xagm.target, usdt.target, amount, 0, 0, now, "0x")
            ).to.be.revertedWithCustomError(xagm, "ERC20InsufficientBalance");
        });
    });
