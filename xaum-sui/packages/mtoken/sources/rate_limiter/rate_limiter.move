// Copied from
// https://github.com/LayerZero-Labs/LayerZero-v2/blob/main/packages/layerzero-v2/sui/contracts/oapps/oft/oft/sources/internal/rate_limiter.move
//
// We aim to minimize changes to the original code. The only modification
// is that `try_consume_rate_limit_capacity` now returns a boolean value
// instead of aborting.
...
public(package) fun try_consume_rate_limit_capacity(self: &mut RateLimiter, eid: u32, amount: u64, clock: &Clock): bool {
    if (!self.has_rate_limit(eid)) return true; // ok to consume if no rate limit is set
    ...
    if (rate_limit.in_flight_on_last_update + amount <= rate_limit.limit) {
        rate_limit.in_flight_on_last_update = rate_limit.in_flight_on_last_update + amount;
        return true // ok to consume
    };
